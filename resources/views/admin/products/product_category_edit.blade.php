@extends('admin.layouts.app')
  @section('content')
    ...
    <div class="panel panel-success">
	   ...
	   <form action="{{route('product-categories.update', $id)}}" enctype="multipart/form-data" method="POST">
		{{ csrf_field() }}
		{{ method_field('PUT') }}
                ...
                <img src="{{url('uploads/product_categories/thumbs/'.$category->image)}}" style="width:120px;" />
							...
							<div class="form-group">
								...
								<label class="control-label">@lang('common.status')</label>
									...
									<select name="status" class="form-control">
										<option value="1" @if($category->status == 1) selected @endif >@lang('common.enabled')</option>
										<option value="0" @if($category->status == 0) selected @endif>@lang('common.disabled')</option>
									</select>
						...
						<div class="col-sm-12">
							<div class="form-group">
								...
								<a class="btn btn-danger" href="{{ route('product-categories.index') }}" data-dismiss="modal"><i class="icon-remove"></i> @lang('common.cancel')</a>
							</div>
						</div>
					</div>
				</div>
